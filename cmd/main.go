...
func (cmd *runCommand) Run() error {
	...
	var rediserr error
	cmd.hosts.TraverseByHost(func(h contest.Host, _ []string) bool {
		rediserr = cmd.startRedisContainer(h)
		if rediserr != nil {
			return false
		}
...
func (cmd *runCommand) evaluateLog(ctx context.Context, expect contest.ExpectScenario) (bool, error) {
	...
	for i := range current.Actions {
		...
		l := log.With().Stringer("actionid", util.UUID()).Logger()
...
func (cmd *runCommand) startRedisContainer(h contest.Host) error {
	...
	if err := h.RemoveContainer(context.Background(), name, dockerTypes.ContainerRemoveOptions{
		RemoveVolumes: true,
		Force:         true,
	}); err != nil {
...
func (cmd *runCommand) startNode(ctx context.Context, alias string) error {
	...
	config := &container.Config{
		...
		AttachStdout: true,
		AttachStderr: true,
		Cmd:          []string{"bash", "-c", "while :; do date; sleep 1; done"}, // FIXME
	}
	...
	if err := host.StartContainer(ctx, config, hostconfig, nil, name); err != nil {
		return e(err, "")
	}
